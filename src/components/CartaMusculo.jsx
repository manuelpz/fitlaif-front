...
export default function CartaMusculo({ ejerciciosElegidos }) {
  ...
  const fetchUltimoPr = async () => {
    const response = await fetch(
      `http://fitlaif-back-production.up.railway.app/ejerciciosPersonalizados/seriesRepeticiones/${ejerciciosElegidos[visibleDiv].ejercicio}`
    )
    if (response.ok) {
      const data = await response.json()
      setUltimasMarcas(data.seriesRepes)
      setUltimoPr(data.pr)
      if (ultimoPr != null && ultimoPr < data.pr) {
        setNuevoPr(true)
        console.log(ultimoPr, data.pr)
      }
    } else setUltimoPr(0)
  }
